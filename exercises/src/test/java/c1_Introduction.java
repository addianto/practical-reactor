...
public class c1_Introduction extends IntroductionBase {
    ...
    public void fortune_top_five() {
        ...
        List<String> results = serviceResult.collect(Collectors.toList())
                .block();
    ...
    public void nothing_happens_until_you_() throws InterruptedException {
        ...
        // Answer: We want to ensure that all data items have been inserted into `companyList` after the pipeline
        // is triggered at line 134 (subscribe()). If there is no Thread.sleep() at line 136, then it is possible
        // that `companyList` is still empty or only contain partial result. So we wait for 1 seconds to ensure
        // the array list contain the expected data items.
    ...
    public void leaving_blocking_world_behind() throws InterruptedException {
        ...
        fortuneTop5()
                .subscribe(companyList::add, null, () -> serviceCallCompleted.setRelease(true));
